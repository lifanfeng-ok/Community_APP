...
public class time_squat extends Activity {
    public Button train;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_time_squat);
        sensormanager=(SensorManager)getSystemService(Context.SENSOR_SERVICE);
        if(sensormanager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER)!=null){
            sensor=sensormanager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);
        }else {
            Toast.makeText(time_squat.this, "没有陀螺仪传感器", Toast.LENGTH_LONG).show();
        }
        video= (VideoView)findViewById(R.id.video2);
        back = (ImageView)findViewById(R.id.fit_back2);
        mediaController = new MediaController(this);
        String uri="android.resource://" + getPackageName() + "/" + R.raw.a4;
        video.setMediaController(mediaController);
        mediaController.setPadding(0,0,0,0);
        video.setVideoURI(Uri.parse(uri));
        train =(Button)findViewById(R.id.train2);
        time_rest = (TextView)findViewById(R.id.time_rest2);
        rl1 = (RelativeLayout)findViewById(R.id.rl_1);
        rl2 = (RelativeLayout)findViewById(R.id.rl_2);
        rest_time = (TextView)findViewById(R.id.rest_time2);
        situp_number = (TextView)findViewById(R.id.squat_number);
        number = (TextView)findViewById(R.id.number2);
        select_time = (EditText)findViewById(R.id.select_time2);
        logo = (ImageView)findViewById(R.id.time_logo2);
    ...
    View.OnClickListener listener=new View.OnClickListener(){
        ...
        public void onClick(View v) {
            if(v.getId() == R.id.train2){
                time = select_time.getText().toString().trim();
                if(time.length() == 0){
                    Toast toast = Toast.makeText(time_squat.this,"请选择运动时长", Toast.LENGTH_LONG);
                                ...
                                runOnUiThread(new Runnable() {      // UI thread
                                    ...
                                    public void run() {
                                        ...
                                        if(re_time < 0){
                                            ...
                                            new AlertDialog.Builder(time_squat.this).setTitle("运动结果")//设置对话框标题

                                                    .setMessage("运动时长："+time+"秒， 深蹲个数："+String.valueOf(real_number))
